...
NAMESPACE="gaussian_vggt256text_3d_fullclean_first50_firstframe_all_v1_initstride4_vis12"
SUBSET="data/robotwin2.0/subsets/teacher_vis_quality_12cases.jsonl"
OUT_DIR="artifacts/robotwin_teacher_cache_quality_check_12cases_initstride4_20260713"
LOG="${OUT_DIR}/run.log"

mkdir -p "${OUT_DIR}"

{
  printf '[%s] precompute init_stride=4 teacher cache\n' "$(date '+%F %T')"
  printf 'GPU_ID=%s\n' "${GPU_ID}"
  printf 'namespace=%s\n' "${NAMESPACE}"
  printf 'subset=%s\n' "${SUBSET}"
  ...
  printf '[%s] visualize init_stride=4 teacher cache\n' "$(date '+%F %T')"
  printf 'manifest=%s\n' "${MANIFEST}"

  "${PYTHON_BIN}" scripts/gaussianwam/visualize_robotwin_teacher_cache.py \
    --manifest "${MANIFEST}" \
    --indices 518656 12125 138034 439580 512734 161901 161902 250748 437361 437360 491946 18636 \
    --include-feature-pca \
    --make-video \
    --fps 8 \
    --output-dir "${OUT_DIR}"
...
d = Path("artifacts/robotwin_teacher_cache_quality_check_12cases_initstride4_20260713")
pngs = sorted(d.glob("robotwin_teacher_idx*.png"))
frames = []
for p in pngs:
...
out = d / "robotwin_teacher_cache_quality_check_12cases_initstride4_smooth_1280w_fps8.mp4"
