...
CONFIG="${CONFIG:-configs/gaussianwam/stage1_robotwin_firstframe_all.yaml}"
...
export ROBOTWIN_DATA_ROOT="${ROBOTWIN_DATA_ROOT:-${GAUSSIANWAM_ROOT}/data/robotwin2.0}"
...
if [[ -n "${GPU_LIST}" ]]; then
  ...
  if (( ${#GPU_IDS[@]} == 0 )); then
    echo "GPU_LIST is empty after parsing: ${GPU_LIST}" >&2
    exit 1
  fi
else
  for (( i = 0; i < NUM_WORKERS; i++ )); do
    GPU_IDS+=("$(( GPU_OFFSET + i ))")
  done
fi

NUM_GPUS="${#GPU_IDS[@]}"
if (( NUM_WORKERS <= 0 )); then
  echo "NUM_WORKERS must be positive, got ${NUM_WORKERS}" >&2
  exit 1
fi
if (( NUM_GPUS <= 0 )); then
  echo "No GPUs available after parsing launch arguments" >&2
  exit 1
fi
...
LOG_DIR="${GAUSSIANWAM_ROOT}/data/robotwin2.0/gaussian_teacher_cache/logs/${CACHE_NAMESPACE}_${RUN_TAG}"
mkdir -p "${LOG_DIR}"
...
echo "ROBOTWIN_DATA_ROOT=${ROBOTWIN_DATA_ROOT}" | tee -a "${LOG_DIR}/manager.log"
echo "HF_HOME=${HF_HOME}" | tee -a "${LOG_DIR}/manager.log"
echo "NUM_WORKERS=${NUM_WORKERS} NUM_GPUS=${NUM_GPUS}" | tee -a "${LOG_DIR}/manager.log"
echo "GPU_IDS=${GPU_IDS[*]}" | tee -a "${LOG_DIR}/manager.log"
